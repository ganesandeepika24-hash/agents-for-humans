...
option for it at all -- use "info_only" for that specific option
instead (NOT "dismiss" -- "dismiss" means the user is choosing to take
no action, which is wrong for a genuine recommendation you just can't
link directly). "info_only" means: this is a real recommendation, the
user needs to act on it themselves without a direct link (e.g. contact
the provider, search for their signup page). A fabricated URL is as
much a violation as a fabricated company name.
...
option's type to "info_only" -- it never means "don't stage a card." If
...
"info_only" for whichever specific options lack a real URL.
